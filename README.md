...
2. **Transform** it via [`dsql-lint`](https://crates.io/crates/dsql-lint)
   so it's DSQL-compatible. Notable rewrites:
    - `SERIAL` / `bigserial` → `BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)`.
      pg_dump expands `SERIAL` into a 4-statement quartet
      (`CREATE TABLE` + `CREATE SEQUENCE` + `ALTER SEQUENCE OWNED BY` +
      `ALTER COLUMN SET DEFAULT nextval`); the migrate flow collapses it
      back into a single inline identity column.
    - `FOREIGN KEY` constraints are removed (DSQL doesn't enforce FKs;
