...
Use `export` to read the source cluster directly, then `migrate` the
result into the destination — two commands, no `pg_dump`, no proxy, no
extra flags:

```bash
# 1. Export the source cluster to a plain .sql (connects to DSQL directly).
aurora-dsql-loader export \
  --endpoint source-cluster.dsql.us-east-1.on.aws \
  --output dsql.sql

# 2. Migrate it into the destination cluster.
aurora-dsql-loader migrate \
  --endpoint dest-cluster.dsql.us-east-1.on.aws \
  --source-uri dsql.sql
```

`export` connects to the source the same way the loader already does
(IAM auth via the sqlx connector — no session GUCs or table `LOCK`, so
the stock-`pg_dump` incompatibility never arises). It reads the catalog
and emits DSQL-clean DDL by construction: identity columns are written
inline as `GENERATED BY DEFAULT AS IDENTITY (CACHE n)` (never the
standalone `ADD GENERATED` form DSQL rejects), the primary key is folded
onto the `CREATE TABLE`, and secondary indexes are emitted for
`dsql-lint` to normalize (`USING` stripped, `ASYNC` added) during
`migrate`. No `SET COMPRESSION`, no idioms to clean up.

The identity counter carries over: `export` appends a
`setval(...)` for each identity column, so the destination's next insert
continues past the loaded rows — no manual `RESTART WITH` needed.

**Schema surgery.** Because DSQL has no in-place `ALTER COLUMN` /
`ALTER TABLE ADD CONSTRAINT`, reshaping a table means recreate-and-copy.
The `export` output is a plain, editable `.sql` — change a column type,
add a constraint, rename something, then `migrate` the edited file into
a fresh cluster.

**Options:** `--schema NAME` and `--table NAME` narrow the export to a
single schema or table; omit `--output` to stream the dump to stdout.

> `export` covers tables, columns (with exact type modifiers), `NOT NULL`,
> defaults, primary keys, identity columns, secondary indexes, and row
> data. It does not export views, foreign keys (DSQL has none), grants,
> or triggers.
