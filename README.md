...
2. **Transform** via [`dsql-lint`](https://crates.io/crates/dsql-lint).
   Rewrites:
    - `SERIAL` / `BIGSERIAL` (pg_dump's 4-statement quartet) → inline
      `BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)`.
    - `FOREIGN KEY` removed (DSQL doesn't enforce FKs; flagged in report).
    - `CREATE INDEX` → `CREATE INDEX ASYNC`, `USING btree` stripped.
    - Standalone `ALTER TABLE ADD CONSTRAINT ... UNIQUE` folded back
      onto the matching `CREATE TABLE`.
    - `JSONB` → `JSON`, etc.
