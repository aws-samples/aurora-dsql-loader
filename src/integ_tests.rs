...
mod tests {
    ...
    /// README highlights — here we add a `NOT NULL DEFAULT` column, which DSQL
    /// cannot do with an in-place `ALTER`), then `migrate` the edited file
    /// back into the same cluster under a new name. Verifies the export
    /// reader, the edit-then-load path, and identity-counter continuation
    ...
    async fn export_then_migrate_dsql_to_dsql_with_edit() -> anyhow::Result<()> {
        ...
        //   2. add a `NOT NULL DEFAULT` column to the CREATE TABLE — the
        //      kind of change DSQL refuses as an in-place ALTER, which is the
        //      whole reason to export/edit/migrate.
        let dump_text = std::fs::read_to_string(&dump_path)?;
        let edited = dump_text
            ...
            .replace(&format!("{src}_id_seq"), &format!("{dst}_id_seq"))
            // Inject a new column after the identity PK line. The COPY block
            // doesn't list it, so DSQL applies the DEFAULT on load.
            .replace(
        ...
        // The injected column landed with its DEFAULT applied to every row.
        let (active_count,): (i64,) = sqlx::query_as(&format!(
            "SELECT COUNT(*) FROM {dst} WHERE status = 'active'"
        ))
        .fetch_one(&dsql_pool)
        .await?;
        assert_eq!(
            active_count, 3,
            "edited NOT NULL DEFAULT column must apply to all rows"
        );
