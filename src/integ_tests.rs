...
mod tests {
    ...
    /// End-to-end proof for the migrate flow: take a real `pg_dump` (full
    /// dump, including DDL) of a schema with the constructs DSQL rejects
    /// out of the box — SERIAL PK, FOREIGN KEY, sync CREATE INDEX,
    /// NOT NULL DEFAULT — pipe it through `run_migrate`, and confirm the
    /// destination schema is DSQL-compatible:
    ///
    /// - The PK columns are `is_identity = 'YES'` (the SERIAL idiom got
    ///   collapsed into an inline `BIGINT GENERATED BY DEFAULT AS IDENTITY`).
    /// - No FOREIGN KEY constraint exists on the dst schema (dsql-lint
    ///   auto-removed it).
    /// - Row counts match the source.
    ///
    /// Gated on `PGDUMP_E2E_SOURCE_URL` so the test runs in CI (where the
    /// postgres service container is available) and skips locally. Uses
    /// the same PG instance as both source and destination — `run_migrate`
    /// rewrites objects under unique `_dst_<suffix>` names so concurrent
    /// runs don't collide.
    #[tokio::test]
    async fn pgdump_migrate_real_full_dump_collapses_serial_strips_fk() -> anyhow::Result<()> {
        ...
        let Some(source_pg_url) = std::env::var("PGDUMP_E2E_SOURCE_URL")
            ...
            eprintln!(
                "skipping pgdump_migrate_real_full_dump_collapses_serial_strips_fk: \
                 PGDUMP_E2E_SOURCE_URL not set"
            );
            return Ok(());
        };
        ...
        // Use a short suffix in object names; pg_dump emits the full
        // schema-qualified names, and we'll rewrite them in the dump
        // text so the loader's apply stage creates `dst_*` objects
        // alongside the (still-existing) source objects in the same DB.
        let suffix = uuid::Uuid::new_v4().simple().to_string();
        let users_src = format!("e2e_users_{suffix}");
        let events_src = format!("e2e_events_{suffix}");
        let users_dst = format!("dst_e2e_users_{suffix}");
        let events_dst = format!("dst_e2e_events_{suffix}");

        // Source schema: SERIAL PK on both, FK from events.user_id ->
        // users.id, sync CREATE INDEX on events.label, NOT NULL DEFAULT
        // ''. Each construct is a separate dsql-lint rule, so a regression
        // in any of them surfaces here.
        sqlx::query(&format!(
        ...
        // Full dump (no --data-only) so we get the embedded DDL the
        // migrate flow needs.
        let dump_dir = tempfile::tempdir()?;
        let dump_path = dump_dir.path().join("dump.sql");
        let status = Command::new("pg_dump")
        ...
        // Rewrite source object names to dst names so apply doesn't
        // collide with the source objects in the same DB. Replace the
        // longer names first so a `users` substring inside an `events`
        // identifier is never mis-targeted.
        let mut dump_text = std::fs::read_to_string(&dump_path)?;
        for (src_name, dst_name) in [(&events_src, &events_dst), (&users_src, &users_dst)] {
            dump_text = dump_text.replace(src_name.as_str(), dst_name.as_str());
        }
        let rewritten_path = dump_dir.path().join("rewritten.sql");
        std::fs::write(&rewritten_path, dump_text)?;

        // Run the migrate flow against the same PG pool (acts as the
        // "DSQL" target). Pool::from_pg_pool wraps the existing sqlx
        // pool so test_pool short-circuits build_pool.
        let test_pool = Pool::from_pg_pool(pg_pool.clone());
        let args = MigrateArgs {
            endpoint: "ignored.dsql.us-east-1.on.aws".to_string(),
        ...
        // exposes, so this assertion holds for both targets.
        let (users_id_is_identity,): (String,) = sqlx::query_as(&format!(
            "SELECT is_identity FROM information_schema.columns \
             WHERE table_schema='public' AND table_name='{users_dst}' AND column_name='id'"
        ))
        .fetch_one(&pg_pool)
        .await?;
        assert_eq!(
            users_id_is_identity, "YES",
            "{users_dst}.id must be is_identity=YES post-migrate"
        );
        let (events_id_is_identity,): (String,) = sqlx::query_as(&format!(
            "SELECT is_identity FROM information_schema.columns \
             WHERE table_schema='public' AND table_name='{events_dst}' AND column_name='id'"
        ))
        .fetch_one(&pg_pool)
        .await?;
        assert_eq!(
            events_id_is_identity, "YES",
            "{events_dst}.id must be is_identity=YES post-migrate"
        );

        // FK absent on dst.
        let (fk_count,): (i64,) = sqlx::query_as(&format!(
            "SELECT COUNT(*) FROM information_schema.table_constraints \
             WHERE table_schema='public' AND table_name='{events_dst}' \
        ...
        let (users_count,): (i64,) =
            sqlx::query_as(&format!("SELECT COUNT(*)::BIGINT FROM {users_dst}"))
                .fetch_one(&pg_pool)
                .await?;
        let (events_count,): (i64,) =
            sqlx::query_as(&format!("SELECT COUNT(*)::BIGINT FROM {events_dst}"))
                .fetch_one(&pg_pool)
                .await?;
        assert_eq!(users_count, 2, "{users_dst} row count mismatch");
        assert_eq!(events_count, 3, "{events_dst} row count mismatch");

        // Cleanup. Best-effort.
        let _ = sqlx::query(&format!(
            "DROP TABLE IF EXISTS {events_src}, {users_src}, {events_dst}, {users_dst} CASCADE"
        ))
        .execute(&pg_pool)
        .await;
        Ok(())
